...
```{r, workflow, out.width = "50%", echo = FALSE}
...
genome(lusc[["LUSC_Mutation-20160128"]]) <- "NCBI37"
seqlevelsStyle(lusc[["LUSC_Mutation-20160128"]]) <- "UCSC"

gnome <- genome(lusc[["LUSC_Mutation-20160128"]])
gnome <- translateBuild(gnome)
genome(lusc[["LUSC_Mutation-20160128"]]) <- gnome

#convert mutation into binary matrix
lusc2 <- qreduceTCGA(lusc)

#remove normal samples and just focus on primaries
tnmae <- TCGAsplitAssays(lusc2, c("01", "11"))
